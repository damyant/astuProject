...
%{--Page for showing list of students whose semester registration is pending,
    & provide VIEW button to show registration details--}%
<html>
<head>
...
<div id="main">
    <fieldset class="form">
        <h3>List of  Students for semester registration</h3>
        ...
        <table class="inner university-size-full-1-1" id="courseTable">
            %{--<input type="text" name="searchStudent" id="searchStudent" placeholder="Search by Roll No" size="21" maxlength="120"><input type="button" onclick="searchRegistrationToApprove()" value="search">--}%
            <thead>
            <tr>
            ...
            <g:each in="${studentList}" status="i" var="student">
                <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                    ...
                    <td> <g:link controller="hod" action="viewSemesterStudent" params="[studentID:student.id]" class="university-text-decoration-none"><button class="university-button">View</button></g:link></td>
                </tr>
            </g:each>
            </tbody>
        </table>
