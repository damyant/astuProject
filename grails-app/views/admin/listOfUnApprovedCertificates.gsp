...
%{--Shows request pending for certificates  & provide button for generating corresponding certificate--}%
<html>
<head>
...
<div id="main">
    <fieldset class="form">
        <h3>List of Certificate Requests</h3>
        ...
        <table class="inner university-size-full-1-1 tableData university-table-text-left" id="courseTable" >
            ...
            <g:each in="${nameList}" status="i" var="listInst">
                <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                    ...
                    <td>${remarks[i]}</td>
                    <td>
                        <g:link controller="certificateTemp" action="loadCorrectMethod" target="_blank" id="${requestId[i]}" params="[certificate:certificateTypeIdList[i]]"><button class="btn btn-default">Generate</button></g:link>
                    </td>
                </tr>
            </g:each>
            </tbody>
